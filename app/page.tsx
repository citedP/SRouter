...
type PricingMode = "free" | "fixed" | "custom" | "unknown";
...
function UsageOverview({ summary }: { summary: UsageSummary | null }) {
  ...
  return <section className="usage-panel card"><div className="section-head"><div><h2>Token & cost observability</h2><div className="muted small">Metadata only — prompt dan respons tidak disimpan.</div></div><span className="tag">This month</span></div><div className="usage-grid"><div className="usage-card"><span className="muted">Input tokens</span><strong>{formatTokens(month.inputTokens)}</strong><small>{month.estimatedUsageRequests ? `${month.estimatedUsageRequests} estimated` : "Provider reported"}</small></div><div className="usage-card"><span className="muted">Output tokens</span><strong>{formatTokens(month.outputTokens)}</strong><small>{formatTokens(month.totalTokens)} total</small></div><div className="usage-card"><span className="muted">Billable cost</span><strong>${month.billableUsd.toFixed(4)}</strong><small>{month.unknownCostRequests ? `${month.unknownCostRequests} unknown pricing` : "Known pricing only"}</small></div></div>{month.requests === 0 && <div className="usage-empty muted">Belum ada usage. Request berikutnya akan tercatat jika metadata logs aktif.</div>}{summary.byProvider.length > 0 && <div className="usage-breakdown">{summary.byProvider.slice(0, 4).map((item) => <div key={item.name}><span>{item.name}</span><span className="mono">{formatTokens(item.totalTokens)} · ${item.billableUsd.toFixed(4)}</span></div>)}</div>}</section>;
}
