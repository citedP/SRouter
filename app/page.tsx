...
function UsageOverview({ summary }: { summary: UsageSummary | null }) {
  ...
  return <section className="usage-panel card"><div className="section-head usage-head"><h2>Usage</h2><span className="tag">Bulan ini</span></div><div className="usage-grid"><div className="usage-card"><span className="usage-label"><ArrowDownToLine size={14} />Masuk</span><strong>{formatTokens(month.inputTokens)}</strong><small>{month.estimatedUsageRequests ? "~ token" : "token"}</small></div><div className="usage-card"><span className="usage-label"><ArrowUpFromLine size={14} />Keluar</span><strong>{formatTokens(month.outputTokens)}</strong><small>{formatTokens(month.totalTokens)} total</small></div><div className="usage-card"><span className="usage-label"><CircleDollarSign size={14} />Estimasi</span><strong>${month.billableUsd.toFixed(4)}</strong><small>{month.unknownCostRequests ? `${month.unknownCostRequests} tanpa harga` : "nilai pasar"}</small></div></div>{month.requests === 0 && <div className="usage-empty muted">Belum ada usage.</div>}{summary.byProvider.length > 0 && <div className="usage-breakdown">{summary.byProvider.slice(0, 4).map((item) => <div key={item.name}><span>{item.name}</span><span className="mono">{formatTokens(item.totalTokens)} · ${item.billableUsd.toFixed(4)}</span></div>)}</div>}</section>;
}
